...
export const meta = {
  editUrl: 'pages/docs/api/api-docs-mdx/endpoints/deployments.mdx',
...
<b>Deploying again with the same request would yield the same response</b>, with
the same ID and URL due to no differences to the files or configuration.
...
<P>
  <b>
    Deploying again with the same request using the{' '}
    <InlineCode>?forceNew=1</InlineCode> URL parameter would yield a new,
    different, response
  </b>
  , with a different ID and URL:
</P>
