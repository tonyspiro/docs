...
You can run build tasks by creating a `vercel-build` script within a `package.json` file at the entry point's level or higher. For example:
...
<Caption>
  ...
  <InlineCode>vercel-build</InlineCode> script to execute in the build step.
</Caption>
...
<Caption>
  Using the{' '}
  <Link href="/docs/deployments/official-runtimes/node-js-now-node/">
