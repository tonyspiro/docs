...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Production**  | When selected, the Environment Variable will be applied to your next Production Deployment. To create a Production Deployment, push a commit to the [Production Branch](/docs/v2/git-integrations#production-branch) or run `vercel --prod`. |
...
Another option for configuring environment variables is to use a `.env` file, which is a file that contains environment variables on new lines in the form of `NAME=VALUE`. For example:
...
Some frontend frameworks, for example [Next.js](https://nextjs.org/docs/basic-features/environment-variables#environment-variables-on-vercel), support loading environment variables from a `.env` file out-of-the-box.

For Node.js serverless functions, you may use the [`dotenv`](https://npmjs.com/dotenv) module in your code to enable loading environment variables from a `.env` file.

<Note>
  When deploying to Vercel, environment variables configured in the Environment
  Variables settings for your project will take precedence over values defined
