...
export const meta = {
  ...
  editUrl: 'pages/docs/custom-domains.mdx',
  lastEdited: '2020-08-14T08:50:54.000Z',
}
...
In this document, adding a domain to your <ProductName /> [projects](/docs/platform/projects/) is explained in detail.

<Note label={false}>
  If you want to migrate an <b>existing domain</b> to Vercel, read our Knowledge
  Base article on{' '}
  <Link href="/knowledge/zero-downtime-migration-for-dns">
    Zero-Downtime Migration for DNS
  </Link>
  .
</Note>
...
/>
<Caption>
  Selecting the <b>Domains</b> menu item from the Project Settings page.
</Caption>
...
Once the domain has been added, you will be presented with different methods for configuring it, depending on whether it is an [**apex domain**](/docs/platform/glossary#apex-domain) (example.com) or **subdomain** (docs.example.com).
...
You can configure [apex domains](/docs/platform/glossary#apex-domain) either with a **A** record or through the **Nameservers** method.
...
/>
<Caption>
  Instructions on configuring an <strong>apex domain</strong> from the Project
  Domains page.
</Caption>
...
/>
<Caption>
  Instructions on configuring a <strong>subdomain</strong> from the Project
  Domains page.
</Caption>
...
/>
<Caption>
  Selecting the <b>Domains</b> tab from the <OrgName /> Dashboard page.
</Caption>
...
<Note type="warning">
  If using your <b>Custom Domain</b> as a <b>Wildcard</b> Domain, you{' '}
  <b>must use the nameservers method for verification</b>.
</Note>

To add a **Wildcard Domain**, follow the steps to [add a domain](#step-1:-selecting-your-project). At [step 3](#step-3:-entering-your-chosen-domain), use the prefix `*` (e.g. `*.acme.com`).
...
/>
<Caption>
  A <b>Wildcard Domain</b> being deployed.
</Caption>
...
When a **Custom Domain** is assigned to a project with an enabled [<ProductName /> for Git Integration](/docs/git-integrations), each push (including merges) to the [Production Branch](/docs/git-integrations#production-branch) (commonly `main`) will trigger a deployment to the defined domain.
...
/>
<Caption>
  A <b>domain redirect</b> that redirects requests made to{' '}
  <InlineCode>www.example.com</InlineCode> to{' '}
...
/>
<Caption>
  Selecting which domains to move from the <b>Domains</b> tab.
</Caption>
...
Every commit pushed to the [Production Branch](/docs/git-integrations#production-branch) of your Git repository using our [Git Integration](/docs/git-integrations) will be assigned the domains [configured in your project](#adding-a-domain).
...
/>
<Caption>
  A <strong>Custom Domain</strong> that is automatically assigned to the{' '}
  <InlineCode>staging</InlineCode> <b>Git Branch</b>.
</Caption>
