...
export const meta = {
  ...
  editUrl: 'pages/docs/platform/deployments.mdx',
...
When merging to the [Production Branch](/docs/v2/git-integrations#production-branch) (commonly `main`), a [production deployment](/docs/v2/platform/deployments/#production) will be made.
...
<Caption>
  Making a preview deployment with the <InlineCode>vercel</InlineCode> command.
</Caption>
...
<Caption>
  Making a production deployment with the <InlineCode>vercel</InlineCode>{' '}
  command.
</Caption>
...
**Production deployments** are made in two different circumstances. Each time you merge to the [Production Branch](/docs/v2/git-integrations#production-branch) (commonly `main`) or make a deployment using the `vercel --prod` command, this is a **production deployment**.
...
- **Force a new deployment** by instructing your client to do so.
  ...
  - If using our API, include the [`forceNew` query parameter](/docs/api#endpoints/deployments/create-a-new-deployment)
