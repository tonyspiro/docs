...
export const meta = {
  ...
  editUrl: 'pages/docs/serverless-functions/edge-caching.mdx',
...
Responses with a HTTP status code of `200` to a `GET` or `HEAD` request are cached by the <CdnName /> for the requested path. Other status codes are never cached.

Additionally, the following are _not_ cached:
