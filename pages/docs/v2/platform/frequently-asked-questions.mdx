...
export const meta = {
  ...
  lastEdited: '2020-07-31T10:44:41.000Z'
}
...
It is no longer required to manually verify domains via TXT records anymore.

Instead, a domain's DNS settings must be configured after the domain is added to a Project. See the <Link href="/docs/v2/custom-domains#step-4:-configuring-the-domain">Custom Domains</Link> section for more information.

## Leaving The Team

You cannot leave a team if you are the last remaining owner of the team or the last confirmed member. In order to leave the team, you first have to designate another confirmed member as an owner of the team.
