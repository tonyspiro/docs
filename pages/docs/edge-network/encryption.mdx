...
export const meta = {
  ...
  editUrl: 'pages/docs/edge-network/encryption.mdx',
...
The unique URLs generated when creating a deployment are handled using a wildcard certificate issued for the `.vercel.app` domain. The **<ProductName />** platform generates wildcard certificates using [LetsEncrypt](https://letsencrypt.org/) and keeps them updated automatically.
